...
- `DATA/dictionary_sorted_by_type.txt` — master dictionary (type sections, single source); the category split canon lives next to it: `DATA/action.dict`, `DATA/mapping.dict`, `DATA/operators.dict`, `DATA/abstracts.txt` (generated by `validator/spellcheck.py`, tracked). `DATA/protos.txt` — abstract prototypes (proto/blplan: formal plans, execution templates); `DATA/blueprints.txt` — ready architectural models (blueprint: ready vector rules of projects/modules); `DATA/agents.txt` — agent rules. Semantic difference: prototypes are the ABSTRACT of ready models, blueprints are the READY (physically complete) models pulled in via `incld="<blueprint-id>"`.
